...
void CTileMap::Render()
{
	...
	HPEN hPen = CreatePen(PS_SOLID, 1, RGB(0, 255, 0)); // 초록색 펜
	...
	for (int Row = LeftTopRow; Row < RightBotRow; ++Row)
	{
		...
		{
			// 타일 테두리 좌표 계산
			int left = (int)(OwnerRenderPos.x + Col * TILE_SIZE);
			int top = (int)(OwnerRenderPos.y + Row * TILE_SIZE);
			int right = (int)(OwnerRenderPos.x + (Col + 1) * TILE_SIZE);
			int bottom = (int)(OwnerRenderPos.y + (Row + 1) * TILE_SIZE);

			// 타일의 테두리만 그리기
			MoveToEx(dc, left, top, nullptr);
			LineTo(dc, right, top);   // 윗변
			LineTo(dc, right, bottom); // 오른쪽 변
			LineTo(dc, left, bottom); // 아랫변
			LineTo(dc, left, top);    // 왼쪽 변
		}
	}

	SelectObject(dc, hOldPen);  // 펜 원래대로 돌리기
	DeleteObject(hPen);         // 펜 메모리 해제
